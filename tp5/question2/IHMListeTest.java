...
public class IHMListeTest extends junit.framework.TestCase{
    ...
    public void test_IHMListe_rechercher() {
        try{
            ...
            assertEquals("IHM modifi?e !!!", components.length, 1);

            //         // la bonne IHM
            assertTrue(components[0] instanceof JPanel);
            Component[] subComponents = ((JPanel)components[0]).getComponents();

            assertTrue("IHM modifi?e, voir JPanel cmd !!!", subComponents[0] instanceof JPanel);// cmd
            assertTrue("IHM modifi?e, voir TextArea cmd !!!", subComponents[1] instanceof TextArea);// texte

            Component[] subSubComponents = ((JPanel)subComponents[0]).getComponents();
            assertTrue("IHM modifi?e, voir JLabel afficheur !!!", subSubComponents[0] instanceof JLabel);// afficheur
            assertTrue("IHM modifi?e, voir JTextField saisie !!!", subSubComponents[1] instanceof JTextField);// saisie
            assertTrue("IHM modifi?e, voir JPanel boutons !!!", subSubComponents[2] instanceof JPanel);// boutons

            Component[] boutons = ((JPanel)subSubComponents[2]).getComponents();
            assertTrue("IHM modifi?e, voir JButton boutonRechercher !!!", boutons[0] instanceof JButton);// boutonRechercher
            assertTrue("IHM modifi?e, voir JButton boutonRetirer !!!", boutons[1] instanceof JButton);// boutonRetirer
            assertTrue("IHM modifi?e, voir JLabel !!!", boutons[2] instanceof JLabel);// 
            assertTrue("IHM modifi?e, voir Checkbox croissant !!!", boutons[3] instanceof Checkbox);// croissant
            assertTrue("IHM modifi?e, voir Checkbox decroissant !!!", boutons[4] instanceof Checkbox);// decroissant
            ...
            assertTrue("\"Java\" doit ?tre pr?sent !!!",res.getText().endsWith("true"));
            ...
            assertTrue("\"Java2\" doit ?tre absent !!!", res.getText().endsWith("false"));
    ...
    public void test_IHM_occurrence() {
        try{
            ...
            try{
                ...
                assertEquals(" Nombre d'occurrence de you est-il erron? ???",36,leNombre);
    ...
    public void test_IHMListe_retirer() {
        try{
            ...
            JLabel res = (JLabel)subSubComponents[0];
            assertTrue("\"you\" doit ?tre pr?sent, pour ce test",res.getText().endsWith("true"));
            ...
            res = (JLabel)subSubComponents[0];
            assertTrue("retrait est-il inop?rant ??? ",res.getText().endsWith("false"));
    ...
    public void test_IHMListe_retirer_et_occurrence() {
        try{
            ...
            JLabel res = (JLabel)subSubComponents[0];
            assertTrue(" ce qui commence par \"lin\" doit ?tre pr?sent, pour ce test",res.getText().endsWith("true"));
            ...
            try{
                ...
                assertTrue(" Nombre d'occurrence de linked est-il erron? ??? (0 apr?s un retrait)",result==0);
            }catch(java.util.InputMismatchException e){
                fail("L'affichage du nombre d'occurrence doit ?tre de 0 apr?s un retrait effectif");
            }
